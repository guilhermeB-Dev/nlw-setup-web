...
export function NewHabitForm() {
  ...
  function handleToggleWeekDay(weekDay: number) {
    if(weekDays.includes(weekDay)) {
      const weekDayWithRemoved = weekDays.filter(day => day !== weekDay)

      setWeekDays(weekDayWithRemoved);
    } else {
  ...
  return(
    <form onSubmit={createNewHabit} className="w-full flex flex-col mt-6">
      ...
      <input type="text" id="title" autoFocus value={ title }
      ...
      <div className="flex flex-col gap-2 mt-3">
        { availableWeekDays.map((weekDay, i) => (
            <Checkbox.Root 
              key={ weekDay } className="flex items-center gap-3 group focus:outline-none"
